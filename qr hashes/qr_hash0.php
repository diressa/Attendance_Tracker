...
//prompt to enter id
//check id % 2
//Display present marked if correct qr code was scanned
//update database
//display invalid entry if incorrect
//contrain the time
...
if(isset($_POST['login'])){//click login button
    ...
    $id_hash = $student_id % 10; //(6 = 1247836 % 10)

    $sql = "SELECT * FROM student WHERE student_id = '$student_id';
            UPDATE student SET present = present + 1 WHERE student_id = '$student_id';";


    if(($conn->multi_query($sql) == TRUE) && (($id_hash - 0) == 0)){
